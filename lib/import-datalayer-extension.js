 ...
 *
 * window.utag_extn.import_datalayer.config = {
...
(function() {
  ...
  // expose the extension in the window.utag_extn.import_datalayer namespace
  window.utag_extn = window.utag_extn || {};
  var utag_extn = window.utag_extn;
  utag_extn.import_datalayer = {};
  
  // expose it in the window.utag_extn.import_datalayer namespace
  utag_extn.import_datalayer.process = function process(config, data) {
    if (toType(config) !== "object") {
      // utag.db("window.utag_extn.import_datalayer: no configuration");
      return {};
    }
    data = data || {};
  ...
  // the configuration should be in window.utag_extn.import_datalayer.config
  var config = utag_extn.import_datalayer.config, b;
  if (config && toType(b) === "object") {
    b = utag_extn.import_datalayer.process(config, b);
  }
